...
export class NousHermesEngine implements ConversationEngine {
  ...
  private readonly logger = new Logger(NousHermesEngine.name);
  ...
  async respond(input: ConversationTurnInput): Promise<ConversationTurnResult> {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        },
        {
          headers: {
            ...
            'X-Hermes-Trace-Id': input.inboundMessageId,
            'X-Hermes-Conversation': this.opaqueConversationId(
      ...
      if (
        validated.replyText.includes(configuration.apiKey) ||
        validated.replyText.includes(configuration.identitySecret)
      ) {
      ...
      return {
        ...
        engine: this.id,
        providerModel:
          validated.providerModel === configuration.model
            ? configuration.model
            : 'unknown',
  ...
  private configuration(): {
    ...
    const rawUrl = this.required('NOUS_HERMES_CHAT_COMPLETIONS_URL');
    const apiKey = this.required('NOUS_HERMES_API_KEY');
    const model = this.required('NOUS_HERMES_MODEL');
    const identitySecret = this.required('NOUS_HERMES_IDENTITY_SECRET');
    let url: URL;
    try {
      url = new URL(rawUrl);
    } catch {
      throw new ConversationEngineConfigurationError(
        'NOUS_HERMES_CHAT_COMPLETIONS_URL is invalid',
      );
    }
    if (!['http:', 'https:'].includes(url.protocol)) {
      throw new ConversationEngineConfigurationError(
        'Nous Hermes endpoint must use HTTP or HTTPS',
      );
    }
    if (
      url.protocol !== 'https:' &&
      !this.booleanConfig('NOUS_HERMES_ALLOW_INSECURE_HTTP', false)
    ) {
      throw new ConversationEngineConfigurationError(
        'Plain HTTP is disabled for the Nous Hermes endpoint',
      );
    }
  ...
  private required(key: string): string {
    ...
    if (!value) {
      throw new ConversationEngineConfigurationError(`${key} is required`);
    }
    return value;
  }
